...
const LeftChildren = () => {
  ...
  const { boxFull, link } = styles;
  const { span } = fontLayout;
  const {
    flexCol,
    flexRowBetween,
  ...
  return (
    <>
      <div className={classNames(`${boxFull} `)}>
        ...
        <div
          className={classNames(
            `pt-2 my-0 mr-auto text-left`,
            `${flexCol} space-y-1`
          )}
        >
          <div className={classNames(`${mainText2} text-black font-semibold`)}>
            NGN 2000
          </div>
          <div className={classNames(`${flexRowBetween}`)}>
            <div
              className={classNames(
                `${flexRowCenter} overflow-hidden ${mainText} text-darkWhite space-x-1 `
              )}
            >
              <AiOutlineStar className={`text-darkRed`} />
              <span
                className={`antialiased text-ellipsis break-words whitespace-normal`}
              >
                4.2
              </span>
            </div>
            <div className={classNames(`${span}`, mainText, `text-darkWhite`)}>
              3300 sold
            </div>
          </div>
        </div>
